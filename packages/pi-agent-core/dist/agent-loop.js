...
async function runLoop(currentContext, newMessages, config, signal, stream, streamFn) {
    ...
    while (true) {
        ...
        while (hasMoreToolCalls || pendingMessages.length > 0) {
            ...
            const toolResults = [];
            if (hasMoreToolCalls) {
                const toolExecution = await executeToolCalls(currentContext, message, config, signal, stream);
...
async function executeToolCallsSequential(currentContext, assistantMessage, toolCalls, config, signal, stream) {
    ...
    for (let index = 0; index < toolCalls.length; index++) {
        ...
        if (preparation.kind === "immediate") {
            ...
            results.push(await finalizeExecutedToolCall(currentContext, assistantMessage, preparation, executed, config, signal, stream));
        }
...
async function executeToolCallsParallel(currentContext, assistantMessage, toolCalls, config, signal, stream) {
    ...
    for (const running of runningCalls) {
        ...
        results.push(await finalizeExecutedToolCall(currentContext, assistantMessage, running.prepared, executed, config, signal, stream));
    }
