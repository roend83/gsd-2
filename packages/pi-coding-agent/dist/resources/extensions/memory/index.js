...
export default function memoryExtension(api) {
    let memorySettings;
    try {
    ...
    if (!memorySettings.enabled) {
        api.registerCommand("memory", {
            ...
            handler: async (_args, ctx) => {
                ctx.ui.notify('Memory extraction is disabled. Enable it with: settings.json \u2192 "memory": { "enabled": true }', "info");
            },
        });
        return;
    }
            ...
            switch (subcommand) {
                case "view": {
                    ...
                    if (memory) {
                        ...
                        ctx.ui.notify("No memories extracted yet. Memories are extracted on session startup.", "info");
                    }
                    break;
                }
                case "clear": {
                    const confirmed = await ctx.ui.confirm("Clear Memories", "Delete all extracted memories for this project?");
                    if (confirmed) {
                ...
                case "rebuild": {
                    const confirmed = await ctx.ui.confirm("Rebuild Memories", "Re-extract all memories from session history? This may take a while.");
                    if (confirmed) {
                        (await getStorage()).resetAllForCwd(ctx.cwd);
                        if (existsSync(projectMemoryDir)) {
                            rmSync(projectMemoryDir, { recursive: true, force: true });
                        }
                        ctx.ui.notify("Memory rebuild enqueued. Extraction will run on next session startup.", "info");
                    }
                    break;
                }
                case "stats": {
                ...
                default:
                    ctx.ui.notify(`Unknown subcommand: ${subcommand}. Use: view, clear, rebuild, stats`, "warning");
            }
        },
    });
    // Cleanup on shutdown
