...
async function runPhase2(storage, config, llmCall, workerId) {
    ...
    try {
        ...
        writeFileSync(join(config.memoryDir, "raw_memories.md"), `# Raw Extracted Memories\n\n\`\`\`json\n${JSON.stringify(allMemories, null, 2)}\n\`\`\`\n`);
        // Call LLM for consolidation
        const consolidationPrompt = getPrompt("consolidation").replace("{{memories_json}}", JSON.stringify(allMemories, null, 2));
        const consolidatedMemory = await llmCall("You are a memory consolidation agent. Merge the extracted memories into a clean, organized markdown document.", consolidationPrompt, { maxTokens: 8192 });
 ...
 */
export async function runStartup(storage, config, llmCall) {
    ...
    if (eligible.length > 0) {
        storage.upsertThreads(eligible.map((f) => ({
            threadId: f.threadId,
            filePath: f.filePath,
            fileSize: f.fileSize,
 ...
 */
export function getMemorySummary(memoryDir) {
 ...
 */
export function getFullMemory(memoryDir) {
