 ...
 */
export function computeLineHash(idx, line) {
 ...
 */
export function formatLineTag(line, text) {
 ...
 */
export function formatHashLines(text, startLine = 1) {
...
export class HashlineMismatchError extends Error {
    ...
    static formatMessage(mismatches, fileLines) {
        ...
        const lines = [];
        lines.push(`${mismatches.length} line${mismatches.length > 1 ? "s have" : " has"} changed since last read. Use the updated LINE#ID references shown below (>>> marks changed lines).`);
        lines.push("");
        let prevLine = -1;
...
export function stripNewLinePrefixes(lines) {
    ...
    for (const l of lines) {
        if (l.length === 0)
            continue;
        nonEmpty++;
        if (HASHLINE_PREFIX_RE.test(l))
            hashPrefixCount++;
        if (DIFF_PLUS_RE.test(l))
            diffPlusCount++;
    }
    if (nonEmpty === 0)
        return lines;
 ...
 */
export function parseHashlineText(edit) {
    if (edit === null)
        return [];
...
function maybeAutocorrectEscapedTabIndentation(edits, warnings) {
    for (const edit of edits) {
        ...
        const corrected = edit.lines.map(line => line.replace(/^((?:\\t)+)/, escaped => {
        ...
        warnings.push(`Auto-corrected escaped tab indentation in edit: converted leading \\t sequence(s) to real tab characters`);
    }
}
const MIN_AUTOCORRECT_LENGTH = 2;
function shouldAutocorrect(line, otherLine) {
    if (!line || line !== otherLine)
        return false;
    line = line.trim();
 ...
 */
export function applyHashlineEdits(text, edits) {
    if (edits.length === 0) {
        ...
        switch (edit.op) {
            case "replace": {
                if (!edit.end) {
                    ...
                    if (shouldAutocorrect(trailingReplacementLine, nextSurvivingLine) &&
                    ...
                    if (shouldAutocorrect(leadingReplacementLine, prevSurvivingLine) &&
