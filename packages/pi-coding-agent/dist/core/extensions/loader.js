...
export async function importExtensionModule(parentModuleUrl, specifier) {
...
export async function discoverAndLoadExtensions(configuredPaths, cwd, agentDir = getAgentDir(), eventBus) {
    ...
    if (localDiscovered.length > 0) {
        ...
        if (untrusted.length > 0) {
            process.stderr.write(`[pi] Skipping ${untrusted.length} project-local extension(s) in ${localExtDir} — project not trusted. Use trustProject() to enable.\n`);
        }
