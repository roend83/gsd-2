...
export async function createAgentSession(options = {}) {
    ...
    const defaultActiveToolNames = editMode === "hashline"
    ...
    agent = new Agent({
        ...
        getApiKey: async (provider) => {
            ...
            const hasAuth = modelRegistry.authStorage.hasAuth(resolvedProvider);
            if (hasAuth) {
                throw new Error(`All credentials for "${resolvedProvider}" are temporarily backed off due to rate limiting. ` +
            ...
            if (isOAuth) {
                ...
                if (modelRegistry.authStorage.areAllCredentialsBackedOff(resolvedProvider)) {
                    throw new Error(`Rate limit in effect for "${resolvedProvider}". ` +
