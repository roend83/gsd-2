...
function createTempProject() {
    ...
    // package.json
    fs.writeFileSync(path.join(dir, "package.json"), JSON.stringify({ name: "lsp-test-project", version: "1.0.0" }, null, 2));
    fs.mkdirSync(path.join(dir, "src"));
    // src/math.ts — module with exported functions
    fs.writeFileSync(path.join(dir, "src", "math.ts"), `export function add(a: number, b: number): number {
  return a + b;
}
...
`);
    ...
    fs.writeFileSync(path.join(dir, "src", "main.ts"), `import { add, subtract, Calculator } from "./math";
...
test("LSP integration: typescript-language-server", async (t) => {
    ...
    try {
        ...
        await t.test("hover on 'add' call", async () => {
            ...
            assert.ok(text.includes("add") || text.includes("number"), `hover text should mention 'add' or 'number', got: ${text.slice(0, 200)}`);
        });
        // ---- Go to Definition ----
        await t.test("go to definition of 'add'", async () => {
            ...
            assert.ok(uri.includes("math.ts"), `definition should point to math.ts, got: ${uri}`);
        });
        // ---- References ----
        ...
        await t.test("diagnostics for type error", async () => {
            ...
            const mainDiags = diagNotifications.filter((n) => n.params.uri === mainUri);
            assert.ok(mainDiags.length > 0, "should receive diagnostics for main.ts");
            const lastDiag = mainDiags[mainDiags.length - 1];
            const diagnostics = lastDiag.params
                .diagnostics;
            // Should catch the type error: string assigned to number
            const typeError = diagnostics.find((d) => d.message.includes("not assignable") || d.message.includes("Type"));
            assert.ok(typeError, `should find type error diagnostic, got: ${diagnostics.map((d) => d.message).join("; ")}`);
        });
        // ---- Shutdown ----
...
//# sourceMappingURL=lsp-integration.test.js.map