...
import { ensureFileOpen, getActiveClients, getOrCreateClient, refreshFile, sendRequest, setIdleTimeout, WARMUP_TIMEOUT_MS, } from "./client.js";
import { getServersForFile, loadConfig, hasRootMarkers, resolveCommand } from "./config.js";
...
import { lspSchema, } from "./types.js";
import { applyCodeAction, collectGlobMatches, dedupeWorkspaceSymbols, extractHoverText, fileToUri, filterWorkspaceSymbols, formatCallHierarchyItem, formatCodeAction, formatDiagnostic, formatDiagnosticsSummary, formatDocumentSymbol, formatGroupedDiagnosticMessages, formatLocation, formatSignatureHelp, formatSymbolInformation, formatWorkspaceEdit, hasGlobPattern, readLocationContext, resolveSymbolColumn, sortDiagnostics, symbolKindToIcon, uriToFile, } from "./utils.js";
export { lspSchema } from "./types.js";
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const lspDescription = fsSync.readFileSync(path.join(__dirname, "lsp.md"), "utf-8");
export async function warmupLspServers(cwd) {
    const config = loadConfig(cwd);
    setIdleTimeout(config.idleTimeoutMs);
    const servers = [];
    const lspServers = getLspServers(config);
    const results = await Promise.allSettled(lspServers.map(async ([name, serverConfig]) => {
...
function getLspServersForFile(config, filePath) {
    return getServersForFile(config, filePath);
}
function getLspServerForFile(config, filePath) {
...
async function formatLocationWithContext(location, cwd) {
    ...
    const context = await readLocationContext(uriToFile(location.uri), location.range.start.line + 1, LOCATION_CONTEXT_LINES);
    if (context.length === 0) {
        return header;
    }
...
async function waitForDiagnostics(client, uri, timeoutMs = 3000, signal, minVersion) {
        ...
        async execute(_toolCallId, params, signal, _onUpdate) {
            ...
            if (action === "diagnostics") {
                ...
                if (truncatedGlobTargets) {
                    results.push(`[W] Pattern matched more than ${MAX_GLOB_DIAGNOSTIC_TARGETS} files; showing first ${MAX_GLOB_DIAGNOSTIC_TARGETS}. Narrow the glob or use workspace diagnostics.`);
                }
                for (const target of targets) {
                    ...
                    for (const [serverName, serverConfig] of servers) {
                        ...
                        try {
                            ...
                            const diagnostics = await waitForDiagnostics(client, uri, diagnosticsWaitTimeoutMs, signal, minVersion);
            ...
            if (action === "symbols" && !resolvedFile) {
                ...
                const respondingServers = new Set();
                for (const [workspaceServerName, workspaceServerConfig] of servers) {
                    ...
                    try {
                        ...
                        const workspaceResult = (await sendRequest(workspaceClient, "workspace/symbol", { query: normalizedQuery }, signal));
            ...
            try {
                ...
                switch (action) {
                    ...
                    case "type_definition": {
                        const result = (await sendRequest(client, "textDocument/typeDefinition", {
                            textDocument: { uri },
                            position,
                        }, signal));
                        const locations = normalizeLocationResult(result);
                    ...
                    case "references": {
                        ...
                        if (!result || result.length === 0) {
                            ...
                            const contextualLines = await Promise.all(contextualReferences.map(location => formatLocationWithContext(location, cwd)));
                    ...
                    case "code_actions": {
                        ...
                        if (apply === true && query) {
                            ...
                            const selectedAction = result.find((actionItem, index) => (parsedIndex !== null && index === parsedIndex) ||
                            ...
                            const appliedAction = await applyCodeAction(selectedAction, {
                                ...
                                executeCommand: async (commandItem) => {
                                    await sendRequest(client, "workspace/executeCommand", {
                    ...
                    case "incoming_calls": {
                        ...
                        const incomingResult = (await sendRequest(client, "callHierarchy/incomingCalls", { item: prepareResult[0] }, signal));
                    ...
                    case "outgoing_calls": {
                        ...
                        const outgoingResult = (await sendRequest(client, "callHierarchy/outgoingCalls", { item: prepareResult[0] }, signal));
                    ...
                    case "format": {
                        ...
                        const formatResult = (await sendRequest(client, "textDocument/formatting", {
                            textDocument: { uri },
                            options: {
                                tabSize: tab_size ?? 4,
                    ...
                    case "signature": {
                        const sigResult = (await sendRequest(client, "textDocument/signatureHelp", {
                            textDocument: { uri },
                            position,
                        }, signal));
