...
export function registerInteractionTools(pi, deps) {
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            selector: Type.Optional(Type.String({ description: "CSS selector of the element to click. The tool will try getByRole fallbacks if the CSS selector fails (handles shadow DOM)." })),
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
            ...
            try {
                ...
                const afterTargetState = params.selector
                    ? await deps.captureClickTargetState(target, params.selector)
                    : null;
                const targetStateChanged = !!beforeTargetState && !!afterTargetState && (beforeTargetState.exists !== afterTargetState.exists ||
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            selector: Type.Optional(Type.String({ description: "CSS selector of the input to type into (clicks it first). Examples: 'input[name=q]', 'textarea', 'combobox[aria-label=\"Search\"]'. The tool will try getByRole fallbacks if the CSS selector fails." })),
                ...
                if (params.selector) {
                    if (params.slowly) {
                        ...
                        if (!filled) {
                            ...
                            await target.locator(":focus").pressSequentially(params.text, { timeout: 5000 }).catch(() => p.keyboard.type(params.text));
                        }
                    ...
                    await target.locator(":focus").pressSequentially(params.text, { timeout: 10000 }).catch(() => p.keyboard.type(params.text));
                }
                if (params.submit) {
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            width: Type.Optional(Type.Number({ description: "Custom viewport width in pixels (requires height too)" })),
            height: Type.Optional(Type.Number({ description: "Custom viewport height in pixels (requires width too)" })),
        }),
        async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
            try {
                const { page: p } = await deps.ensureBrowser();
