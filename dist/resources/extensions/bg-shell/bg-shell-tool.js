...
export function registerBgShellTool(pi, state) {
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            label: Type.Optional(Type.String({ description: "Short human-readable label for the process (for start)" })),
            id: Type.Optional(Type.String({ description: "Process ID (for digest, output, highlights, wait_for_ready, send, send_and_wait, run, signal, kill, restart)" })),
            stream: Type.Optional(StringEnum(["stdout", "stderr", "both"])),
            tail: Type.Optional(Type.Number({ description: "Number of most recent lines to return (for output). Defaults to 100." })),
            filter: Type.Optional(Type.String({ description: "Regex pattern to filter output lines (for output). Case-insensitive." })),
            input: Type.Optional(Type.String({ description: "Text to write to process stdin (for send, send_and_wait)" })),
            wait_pattern: Type.Optional(Type.String({ description: "Regex to wait for in output (for send_and_wait)" })),
            signal_name: Type.Optional(Type.String({ description: "OS signal to send, e.g. SIGINT, SIGTERM, SIGHUP (for signal)" })),
            timeout: Type.Optional(Type.Number({ description: "Timeout in milliseconds (for wait_for_ready, send_and_wait, run). Default: 30000 for wait_for_ready/send_and_wait, 120000 for run" })),
            type: Type.Optional(StringEnum(["server", "build", "test", "watcher", "generic", "shell"])),
            ready_pattern: Type.Optional(Type.String({ description: "Regex pattern that indicates the process is ready (for start)" })),
            ready_port: Type.Optional(Type.Number({ description: "Port to probe for readiness (for start). When open, process is considered ready." })),
            ready_timeout: Type.Optional(Type.Number({ description: "Max milliseconds to wait for ready_port/ready_pattern before marking as error (default: 30000)" })),
            group: Type.Optional(Type.String({ description: "Group name for related processes (for start, group_status)" })),
            ...
            switch (action) {
                ...
                case "wait_for_ready": {
                    ...
                    if (ready) {
                        ...
                        return new Text(theme.fg("error", "✗ Not ready ") + theme.fg("accent", proc.id) + " " + theme.fg("dim", String(details.detail)), 0, 0);
                    }
                }
                case "send": {
                    const proc = details.process;
                ...
                case "send_and_wait": {
                    ...
                    if (matched) {
                        return new Text(theme.fg("success", "✓ ") + theme.fg("muted", `Pattern matched — ${proc.id}`), 0, 0);
                    }
                    return new Text(theme.fg("warning", "✗ ") + theme.fg("muted", `Timed out — ${proc.id}`), 0, 0);
                }
                case "run": {
                    const proc = details.process;
                ...
                case "kill": {
                    const proc = details.process;
                    return new Text(theme.fg("success", "✓ Killed ") + theme.fg("accent", proc.id) + " " + theme.fg("muted", proc.label), 0, 0);
                }
                case "restart": {
                    const proc = details.process;
                    return new Text(theme.fg("success", "↻ Restarted ") + theme.fg("accent", proc.id) + " " + theme.fg("muted", proc.label) + " " + theme.fg("dim", `#${proc.restartCount}`), 0, 0);
                }
                case "env": {
                    const proc = details.process;
