        ...
        if (entry.type === "toolCall" || (entry.role === "assistant" && entry.content && Array.isArray(entry.content))) {
            if (entry.type === "toolCall") {
                ...
                if (name === "bash") {
                    ...
                    if (cmd)
                        result.commandsRun.push({ command: cmd.slice(0, 80), failed: false });
                }
            }
        }
        // Count errors
...
async function handleLogsDebug(basePath, ctx, idx) {
    const debugLogs = listDebugLogs(basePath);
    if (debugLogs.length === 0) {
        ctx.ui.notify("No debug logs found.\n\nEnable debug logging: GSD_DEBUG=1 gsd auto", "info");
        return;
    }
    if (idx === undefined) {
        // List debug logs
        const lines = ["Debug Logs (.gsd/debug/):", ""];
        for (let i = 0; i < debugLogs.length; i++) {
            const d = debugLogs[i];
