...
export function createAutoWorktree(basePath, milestoneId) {
    ...
    try {
        ...
        throw new GSDError(GSD_IO_ERROR, `Auto-worktree created at ${info.path} but chdir failed: ${getErrorMessage(err)}`);
    }
...
export function teardownAutoWorktree(originalBasePath, milestoneId, opts = {}) {
    ...
    try {
        ...
        throw new GSDError(GSD_IO_ERROR, `Failed to chdir back to ${originalBasePath} during teardown: ${getErrorMessage(err)}`);
    }
    nudgeGitBranchCache(previousCwd);
...
export function isInAutoWorktree(basePath) {
    ...
    try {
        ...
        if (stat.isDirectory())
            return false; // Main repo has .git dir, not file
...
export function enterAutoWorktree(basePath, milestoneId) {
    ...
    try {
        ...
        throw new GSDError(GSD_IO_ERROR, `Failed to enter auto-worktree at ${p}: ${getErrorMessage(err)}`);
    }
    nudgeGitBranchCache(previousCwd);
    return p;
}
...
export function mergeMilestoneToMain(originalBasePath_, milestoneId, roadmapContent) {
    ...
    try {
        removeWorktree(originalBasePath_, milestoneId, { branch: null, deleteBranch: false });
    }
    catch {
