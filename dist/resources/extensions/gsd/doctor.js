...
async function ensureSliceSummaryStub(basePath, milestoneId, sliceId, fixesApplied) {
...
async function ensureSliceUatStub(basePath, milestoneId, sliceId, fixesApplied) {
...
async function markTaskDoneInPlan(basePath, milestoneId, sliceId, taskId, fixesApplied) {
    ...
    if (!content)
        return;
    const updated = content.replace(new RegExp(`^(\\s*-\\s+)\\[ \\]\\s+\\*\\*${taskId}:`, "m"), `$1[x] **${taskId}:`);
    if (updated !== content) {
        await saveFile(planPath, updated);
        fixesApplied.push(`marked ${taskId} done in ${planPath}`);
    }
}
async function markSliceDoneInRoadmap(basePath, milestoneId, sliceId, fixesApplied) {
    ...
    if (!content)
        return;
    const updated = content.replace(new RegExp(`^(\\s*-\\s+)\\[ \\]\\s+\\*\\*${sliceId}:`, "m"), `$1[x] **${sliceId}:`);
    if (updated !== content) {
        ...
        for (const slice of roadmap.slices) {
            ...
            for (const task of plan.tasks) {
                ...
                if (!task.done && hasSummary) {
                    ...
                    if (fix)
                        await markTaskDoneInPlan(basePath, milestoneId, slice.id, task.id, fixesApplied);
                }
