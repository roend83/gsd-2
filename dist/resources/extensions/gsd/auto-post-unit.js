...
import { resolveSliceFile, resolveTaskFile, resolveMilestoneFile, } from "./paths.js";
...
import { checkPostUnitHooks, getActiveHook, resetHookState, isRetryPending, consumeRetryTrigger, persistHookState, } from "./post-unit-hooks.js";
 ...
 */
function dispatchUnit(s, basePath, unitType, unitId) {
...
export async function postUnitPostVerification(pctx) {
    ...
    if (s.currentUnit?.type === "execute-task" && !s.stepMode) {
        try {
            ...
            process.stderr.write(`gsd-mechanical: completion failed: ${err.message}\n`);
    ...
    if (s.currentUnit && !s.stepMode) {
        ...
        if (hookUnit) {
            ...
            ctx.ui.notify(`Running post-unit hook: ${hookUnit.hookName} (cycle ${hookState?.cycle ?? 1})`, "info");
            // Switch model if the hook specifies one
            if (hookUnit.model) {
                const availableModels = ctx.modelRegistry.getAvailable();
                const match = availableModels.find(m => m.id === hookUnit.model || `${m.provider}/${m.id}` === hookUnit.model);
                if (match) {
                    try {
            ...
            s.unitTimeoutHandle = setTimeout(async () => {
                ...
                ctx.ui.notify(`Hook ${hookUnit.hookName} exceeded ${supervisor.hard_timeout_minutes ?? 30}min timeout. Pausing auto-mode.`, "warning");
                resetHookState();
                await pauseAuto(ctx, pi);
            }, hookHardTimeoutMs);
            if (!s.active)
                return "stopped";
            pi.sendMessage({ customType: "gsd-auto", content: hookUnit.prompt, display: s.verbose }, { triggerTurn: true });
            return "dispatched";
        }
        ...
        if (isRetryPending()) {
            ...
            if (trigger) {
                ...
                ctx.ui.notify(`Hook requested retry of ${trigger.unitType} ${trigger.unitId}.`, "info");
                ...
                if (pending.length > 0) {
                    ...
                    if (mid && sid) {
                        ...
                        s.unitTimeoutHandle = setTimeout(async () => {
                            ...
                            if (!s.active)
                                return;
                            ctx.ui.notify(`Triage unit exceeded timeout. Pausing auto-mode.`, "warning");
                            await pauseAuto(ctx, pi);
                        }, triageTimeoutMs);
                        if (!s.active)
                            return "stopped";
    ...
    if (!s.stepMode &&
        ...
        try {
            ...
            s.unitTimeoutHandle = setTimeout(async () => {
                ...
                if (!s.active)
                    return;
                ctx.ui.notify(`Quick-task ${capture.id} exceeded timeout. Pausing auto-mode.`, "warning");
                await pauseAuto(ctx, pi);
            }, qtTimeoutMs);
            if (!s.active)
                return "stopped";
